...
class ColoredListsUsers
{
	...
	public function createAccount()
	{
		...
		if($stmt = $this->_db->prepare($sql)) {
			...
			$row = $stmt->fetch();
			if($row['theCount']!=0) {
				return "<div class='alertBox'><h2> Error </h2>"
			...
			if(!$this->sendVerificationEmail($u, $v)) {
				return "<div class='alertBox'><h2> Error </h2>"
					...
					. "<a href=\"mailto:help@homeofremembrance.com\">contact "
					. "us</a> for support. We apologise for the "
					. "inconvenience. </p></div>";
			}
			$stmt->closeCursor();
		}
		...
		if($stmt = $this->_db->prepare($sql)) {
			 ...
			 * retrieved, create a default lists.
			 */
			 $sql = "INSERT INTO user_vault (UserID) VALUES 
					(
						(
							SELECT UserID
							FROM users
			...
			$sql = "INSERT INTO user_bucketlist (UserID, ListURL) VALUES 
					(
						(
							SELECT UserID
							FROM users
			...
			if($stmt = $this->_db->prepare($sql)) {
				...
				$stmt->execute();
				$stmt->closeCursor();
				return "<div class='alertBox'><h2> Success! </h2>"
				...
				return "<div class='alertBox'><h2> Error </h2>"
					...
					. "creating your first list failed. </p></div>";
			}
			
			//add entries to vault here
			
			$sql = "";
			
		} else {
			return "<div class='alertBox'><h2> Error </h2><p> Couldn't insert the "
				. "user information into the database. </p></div>";
		}
	}
...
EMAIL;
	...
	{
		$sql = "SELECT Username,Firstname,Surname 
	    		FROM users
	    		WHERE Username=:user
