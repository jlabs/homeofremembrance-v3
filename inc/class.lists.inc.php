...
class BucketListItems
{
	...
	{
		$sql = "SELECT ListID FROM user_bucketlist WHERE UserID=:user";
	...
	{
		$sql = "SELECT
					ListID, ListText, ListItemID, ListItemColor, ListItemDone, ListURL
				FROM 
					user_bucketlist_items
				LEFT JOIN user_bucketlist
				USING (ListID)
				WHERE ListID=(
					SELECT ListID
					FROM user_bucketlist
					WHERE UserID=(
						SELECT UserID
						FROM users
						WHERE Username=:user
	...
	private function formatListItems($row, $order)
	{
		...
		{
			$d = '<img class="crossout" src="/images/crossout.png" style="width: 100%; display: block;"/>';
		}
		else
		{
			$d = NULL;
	...
	public function addListItem()
	{
		...
		//$text = strip_tags(urldecode(trim($_POST['new-list-item-text'])), WHITELIST);
		$pos = $_POST['pos'];
		//$pos = $_POST['new-list-item-position'];
		//$pos = 4;

		$sql = "INSERT INTO user_bucketlist_items
					(ListItemID,ListID,ListText,ListItemPosition,ListItemDone,ListItemColor) 
    			VALUES (NULL,:list,:text,:pos,0,2)";
		try
		{
	...
	public function updateListItem()
	{
		...
		$sql = "UPDATE user_bucketlist_items
				SET ListText=:text
				WHERE ListItemID=:id
				LIMIT 1";
		if($stmt = $this->_db->prepare($sql)) {
	...
	public function deleteListItem()
	{
		...
		$sql = "DELETE FROM user_bucketlist_items
